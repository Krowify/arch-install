...
set -euo pipefail
#-------------------------------------------------------------------------
# Stage 6: Deploy dotfiles and activate the installed themes -- must run
...
ZSHRC="${HOME}/.zshrc"
MARKER=">>> linux-installation dotfiles (stage 6) >>>"
touch "${ZSHRC}"
if grep -qF "${MARKER}" "${ZSHRC}"; then
    echo "~/.zshrc already has the stage 6 block, skipping"
else
